...
class filter:
   def __init__(self, name):
      ...
      self.circ_dist = cv2.createTrackbar('circ_dist','image',20,200,self.callback)
      self.circ_low = cv2.createTrackbar('circ_low','image',2,200,self.callback)
      self.circ_high = cv2.createTrackbar('circ_high','image',10,200,self.callback)
   ...
   def blobs_cb(self):
      ...
      im = cv2.resize(self.img_res, (640,480))
